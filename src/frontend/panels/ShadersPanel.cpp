...
float ComputeCardHeight(bool hasSlider, bool enabled) {
    float sliderH = (hasSlider && enabled) ? kCardSliderH : 0.0f;
    return kCardHeaderH + kCardDescH + sliderH + kCardPadding;
}
                ...
                bool recommended = false)
{
    ...
    const float cardH     = ComputeCardHeight(hasSlider, *enabled);
...
void ShadersPanel::RenderContent() {
    ...
    Effect effects[] = {
        { "fsr", IconUpscale, IM_COL32(90, 170, 245, 255), "FSR 1.0", "Reescala y afila video de baja resolucion.",
    ...
    const int   count    = (int)(sizeof(effects) / sizeof(effects[0]));
    ...
    for (int i = 0; i < count; i += 2) {
        ...
        float leftH  = ComputeCardHeight(left.sliderLabel != nullptr, *left.enabled);
        float rightH = right ? ComputeCardHeight(right->sliderLabel != nullptr, *right->enabled) : 0.0f;
